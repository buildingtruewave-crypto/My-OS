...
def render(ctx):
    intro = (
        ...
        + "Make PULSE yours, manage the clock, "
        + "and move your data.</div>"
    )
    st.markdown(UI.panel("Preferences", intro),
    ...
    with c1:
        st.markdown(
            '<div class="tw-lab" '
            + 'style="margin:4px 0 10px">PROFILE</div>',
            unsafe_allow_html=True,
        )
        name = st.text_input(
            "Display name",
        ...
        st.markdown(
            '<div class="tw-lab" '
            + 'style="margin:18px 0 10px">ACCENT</div>',
            unsafe_allow_html=True,
        )
        ...
        if off != st.session_state.get("tz_offset"):
            ...
            D._save_prefs()
        st.caption(
            "Adjust if the host clock is not your local time."
        )

    with c2:
        st.markdown(
            '<div class="tw-lab" '
            + 'style="margin:4px 0 10px">DATA ENGINE</div>',
            unsafe_allow_html=True,
        )
        seed = st.number_input(
            "Random seed", 1, 9999999999,
            value=int(st.session_state.get("seed",
                                           D.DEFAULT_SEED)),
            step=1, key="s_seed",
        )
        ...
        with ea:
            st.download_button(
                "Export all (.zip)", D.export_zip(),
                file_name="pulse_life.zip",
                mime="application/zip", key="s_zip",
            )
        with eb:
            if st.button("Reset to fresh seed",
                         key="s_reset"):
    ...
    deploy = (
        ...
        + '<span class="v">streamlit run app.py - '
        + 'or Streamlit Cloud</span></div>'
        + '<div class="tw-stat"><span class="k">VPS</span>'
        + '<span class="v">docker compose up -d - or '
        + 'systemd + nginx (see /deploy)</span></div>'
        ...
        + '<span class="tw-accent">colour = outcome - '
        + 'routine = spine</span></div>'
    )
    st.markdown(UI.panel("Deployment", deploy),
                unsafe_allow_html=True)
