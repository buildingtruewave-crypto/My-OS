...
def render(ctx):
    ...
    with c1:
        st.markdown('<div class="tw-lab" style="margin:4px 0 10px">PROFILE</div>',
                    unsafe_allow_html=True)
        name = st.text_input("Display name",
                             value=st.session_state.get("name", D.DEFAULT_NAME), key="s_name")
        if name != st.session_state.get("name"):
            st.session_state["name"] = name
            D._save_prefs()

        st.markdown('<div class="tw-lab" style="margin:18px 0 10px">ACCENT</div>',
                    unsafe_allow_html=True)
        ...
        st.markdown('<div class="tw-lab" style="margin:18px 0 10px">CLOCK OFFSET (HRS)</div>',
                    unsafe_allow_html=True)
        off = st.number_input("offset", -12.0, 14.0,
    ...
    with c2:
        st.markdown('<div class="tw-lab" style="margin:4px 0 10px">DATA ENGINE</div>',
                    unsafe_allow_html=True)
        seed = st.number_input("Random seed", 1, 9999999999,
        ...
        st.markdown('<div class="tw-lab" style="margin:18px 0 10px">MOVE YOUR LIFE</div>',
                    unsafe_allow_html=True)
        ea, eb = st.columns(2)
        with ea:
