...
def render(ctx):
    ...
    st.markdown(UI.panel("Quarterly Targets", intro),
    ...
    if goals:
        ...
        for i, g in enumerate(goals):
            ...
            with cols[i % len(cols)]:
                st.markdown(
                    UI.goal_html(g, pct, color),
                    unsafe_allow_html=True,
                )
    else:
        st.markdown(
            UI.empty_state("No goals yet."),
            unsafe_allow_html=True,
        )
        ...
        if mode == "Add new":
            ...
            with a:
                ...
                metric = st.text_input(
                    "Metric label", "units", key="g_m",
                )
                target = st.number_input(
                    "Target", 0.0, None, 10.0,
                    step=1.0, key="g_tg",
                )
            with b:
                current = st.number_input(
                    "Current", 0.0, None, 0.0,
                    step=1.0, key="g_c",
                )
                quarter = st.text_input(
                    "Quarter / deadline", "Q3", key="g_q",
                )
            ...
            if goals:
                ...
                sel = st.selectbox("Goal", titles,
                                   key="g_sel")
                g = next(x for x in goals
                         if x["title"] == sel)
                nv = st.number_input(
                    "New current value", 0.0, None,
