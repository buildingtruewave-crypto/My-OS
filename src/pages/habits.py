...
def render(ctx):
    ...
    log = ctx["habit_log"]
    today = ctx["today"]
    dates = [today - dt.timedelta(days=o)
             for o in range(34, -1, -1)]
    ...
    intro = (
        ...
        + "Tick today's habits below and the wall "
        + "updates live.</div>"
    )
    st.markdown(
        UI.panel("Consistency Wall - last 35 days", intro),
    ...
    with c1:
        ...
        W.habit_checklist("hab", habits, log,
                          today.isoformat())
    with c2:
        items = []
        for h in habits:
            _d, _t, pct, streak = M.habit_stats(
                log, h["id"], 30,
            )
            if pct >= 70:
                color = "#34D399"
    ...
    txt = st.text_area(
        "habits", value=D.habits_to_text(habits),
        height=200, label_visibility="collapsed",
        key="hb_text",
    )
    a, b, _ = st.columns([1, 1, 4])
    with a:
        if st.button("Save habits", type="primary",
                     key="hb_save"):
