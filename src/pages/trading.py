...
def render(ctx):
    ...
    if trades.empty:
        st.markdown(
            UI.empty_state("No trades logged."),
    ...
    row = [
        UI.tile("Net PnL", U.fmt_money(net, True),
                U.fmt_pct(wr, False), net_tone,
                net_tone, "~", "accent", 0),
        UI.tile("Win Rate", U.fmt_pct(wr, False),
                wl_txt, "mute", "ink", "@", "jewel", 40),
        UI.tile("Trades", str(n), "", "mute", "ink",
                "#", "accent", 80),
        UI.tile("Win Streak", str(ws), "", "win",
                "win", "+", "win", 120),
        UI.tile("Loss Streak", str(ls), "", "loss",
                "loss", "!", "loss", 160),
    ]
    ...
    with c3:
        ...
        st.markdown(
            UI.panel(
                "Recent Trades",
                UI.table(
                    ["Time", "Asset", "Dir", "Entry",
                     "Exit", "Lots", "PnL", "Strategy"],
                    rows,
                ),
            ),
            unsafe_allow_html=True,
