...
def render(ctx):
    ...
    st.markdown(UI.panel("This Week's Spine",
                         ...
                         'Every other page hangs off this schedule. Edit it below - or paste '
                         'your real routine and PULSE re-orients instantly.</div>'),
    ...
    for w in range(7):
        ...
        for b in blocks:
            bt = b["time"]
            bl = _esc(b["label"])
            body.append('<div class="tw-stat"><span class="k" style="font-family:var(--mono)">' +
                        bt + '</span>'
                        '<span class="v" style="font-size:11px;font-family:var(--body);'
                        'font-weight:600;color:var(--ink-2)">' + bl + '</span></div>')
        bhtml = "".join(body) or '<div class="tw-empty">rest</div>'
        with cols[w]:
            st.markdown(UI.panel(names[w], bhtml), unsafe_allow_html=True)
