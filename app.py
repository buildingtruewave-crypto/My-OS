...
from src import data as D, metrics as M, theme, ui as UI, util as U
...
st.set_page_config(page_title="PULSE - Life Command Center", page_icon="o",
...
st.markdown("<style>:root{--accent:%s;--accent-soft:%s;}</style>"
            % (accent, _rgba(accent)), unsafe_allow_html=True)

NAV = [("o", "Now"), ("=", "Routine"), ("v", "Habits"), ("~", "Trading"),
       ("g", "Goals"), ("t", "Tasks"), ("j", "Journal"), ("s", "Stats"), ("x", "Settings")]
OPTIONS = [g + "  " + n for g, n in NAV]
PAGES = {"Now": now, "Routine": routine, "Habits": habits, "Trading": trading,
         "Goals": goals, "Tasks": tasks, "Journal": journal, "Stats": stats,
         "Settings": settings}
...
ctx = dict(today=today, now_dt=now_dt, routine=routine, today_blocks=today_blocks,
           active_idx=active_idx, progress=progress, current=current,
           next_block=next_block, habits=habits, habit_log=log, goals=goals,
           tasks=tasks, journal=journal, trades=trades, trade=trade, start=start,
           accent=accent, name=st.session_state.get("name", D.DEFAULT_NAME))
...
with d:
    st.markdown('<div style="padding:.3rem 0"><div class="tw-lab">today</div>'
                '<div style="font:700 18px/1.1 var(--disp);color:var(--ink)">' +
                day_name + '</div>'
                '<div class="tw-sub">' + day_sub + '</div></div>',
                unsafe_allow_html=True)
with s:
    st.markdown('<div style="text-align:right;padding:.3rem 0"><div class="tw-lab">life score</div>'
                '<div class="tw-val ' + score_tone + '" style="font-size:30px">' +
                str(score) + '</div></div>', unsafe_allow_html=True)
